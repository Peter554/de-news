...
def main():
    ...
    for pr in prs:
        ...
        if changed_files:
            ...
            conflicts = {f: checked_out_by[f] for f in changed_files if f in checked_out_by}
            if conflicts:
                print(f"  ERROR: File conflicts detected with earlier PRs:")
    ...
    Path(TAGS_FILE).write_text(json.dumps(all_tags, indent=2, ensure_ascii=False) + "\n")
